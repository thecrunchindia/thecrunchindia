...
import { Clock, User, Printer, X, Check, MapPin, Loader2 } from 'lucide-react';
        ...
        <div className="px-6 py-2.5 flex justify-between items-center bg-gray-100 border border-gray-100">
          <div className="flex items-center gap-3">
            <span className="text-sm font-black text-gray-900 tracking-tight">
              ORDER <span className="text-[#f9a602]">#{order.id}</span>
            </span>
            <div className="flex items-center gap-1 text-[#f9a602] ml-2">
              <Clock size={12} />
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button 
              ...
              className="cursor-pointer flex-[3] sm:flex-[4] flex items-center justify-center gap-2 py-3 bg-[#f9a602] text-black rounded-xl font-black text-[10px] uppercase shadow-xl disabled:opacity-50"
            >
              {isThisOrderUpdating ? (
              ...
              {isThisOrderUpdating ? 'Processing...' : 'Accept Order'}
            </button>
          </div>
        </div>
      </div>
...
const NewOrders = () => {
  const { orders, isLoading, isError, error, updateStatus, isUpdating } = useNewOrders();
  const [modalData, setModalData] = useState({ isOpen: false, orderId: null });
  const [updatingOrderId, setUpdatingOrderId] = useState(null);

  const handleAccept = (id) => {
    setUpdatingOrderId(id);
    updateStatus({ orderId: id, status: 'PREPARING' });
  };
